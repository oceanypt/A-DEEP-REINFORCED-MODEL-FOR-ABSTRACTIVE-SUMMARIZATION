...
def RL_evaluate_loss(model, data):
    model.eval()
    #cum_loss = 0.
    #cum_tgt_words = 0.i
    samples = 0.
	...
	tgt_sents_len = [len(s[1:]) for s in tgt_sents]
        pred_tgt_word_num = sum(tgt_sents_len) 
        # (tgt_sent_len, batch_size, tgt_vocab_size)
	#123
        #scores = model(src_sents_var, src_sents_len, tgt_sents_var[:-1])
        #loss = crit(scores.view(-1, scores.size(2)), tgt_sents_var[1:].view(-1))
	...
	re_sample += reward_RL_sample
	re_base += reward_RL_base
	...
	samples += len(src_sents)
        #cum_loss += loss.data[0]
        #cum_tgt_words += pred_tgt_word_num

    #loss = cum_loss / cum_tgt_words
    return sum(re_sample) / samples, sum(re_base) / samples
...
def RL_train(args):
    ...
    dev_data_src = read_corpus(nmt_args.dev_src, source = 'src')[:5000]
    dev_data_tgt = read_corpus(nmt_args.dev_tgt, source = 'tgt')[:5000]
    dev_data = zip(dev_data_src, dev_data_tgt)
	...
	for src_sents, tgt_sents in data_iter(train_data, batch_size = args.batch_size):
		...
		#model_file = args.save_to + '.iter%d.bin' % train_iter
                ...
                re_sample, re_base = RL_evaluate_loss(model, dev_data)
                dev_ppl = -re_base   #dev_loss  #np.exp(dev_loss)
		...
		'''
		valid_metric = -dev_ppl
		print ('validation: iter %d, dev. re_sample %f,  dev. re_base %f' % (train_iter, re_sample, re_base), file = sys.stderr)
                model.train()
